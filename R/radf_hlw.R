...
#
# A two-step wrapper around radf_hls()'s already-shipped single-bubble
...
# regime's end and the next one's start). Step 2 applies radf_hls()-
...
# observation (matching radf_hls()'s own idx[b + 1L] convention).
...
#'
#' \code{radf_hlw} extends \code{\link{radf_hls}} to series with more
...
#' is not supplied.
#'
#' @return An object of class \code{radf_hlw_obj}: a list, one element
...
#' Finance, 58, 226-246.
#'
#' @seealso \code{\link{radf_hls}} for the single-bubble fitting this
...
radf_hlw <- function(data, cv = NULL, minw = NULL, trim = 0.1,
  ...
  results %>%
    ...
    add_class("radf_hlw_obj")
}

#' @export
print.radf_hlw_obj <- function(x, digits = max(3L, getOption("digits") - 3L), ...) {
  cat_line()
  cat_rule(left = glue("radf_hlw (n = {attr(x, 'n')}, trim = {attr(x, 'trim')})"))
  for (nm in names(x)) {
    cat_line()
    cat_line(nm, ":")
