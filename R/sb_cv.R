...
#'   autoplot(cv = sb)
#' }
sb_cv <- function(data, minw, nboot = 1000, lag = 0) {
...
sb_dist <- function(data, minw, nboot = 1000, lag = 0) {

  y <- data %>% discard_index() %>% as.matrix()
  nc <- NCOL(y)
  nr <- NROW(y)
  ...
  bsadf_crit <- apply(edf_bsadf_panel, 1, quantile, probs = pr) %>% t()
  gsadf_dist <- apply(edf_bsadf_panel, 2, max) %>% unname()
  gsadf_crit <- quantile(gsadf_dist, probs = pr)
