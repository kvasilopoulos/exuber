...
#' @export
print.report <- function(x, digits = max(3L, getOption("digits") - 3L), ...) {
  cat(
    "\n", "Recursive Unit Root\n",
    "--------------------------------\n",
    "H0:", "Unit root\n",
  ...
  if (attr(x, "panel")) {
    ...
    for (i in seq_along(x)) {
      cat("\n", names(x)[i], "\n")
      print(x[[i]], digits = digits, print.gap = 1L)
    }
  }
...
#' "gsadf".
#' @param bool Logical. If TRUE returns 0 (1) for not rejecting (not rejecting).
#'
...
diagnostics <- function(x, y, option = c("gsadf", "sadf"),
  ...
  if (option == "gsadf") {
    ...
    if (method(y) == "Monte Carlo") {
      ...
      stop("Explosive periods with Bootstraped critical values ",
  ...
  if (bool) {
    ...
    names(sig) <- if (panel) "Panel" else col_names(x)
    return(sig)
  }
...
#' @export
#'
datestamp <- function(x, y, option = c("gsadf", "sadf"), panel = FALSE,
                      min_duration = 0, bool = FALSE) {
  assert_radf(x)
  assert_cv(y, panel)
  assert_minw(x, y)
  assert_nonnegeative_int(min_duration)

  option <- match.arg(option)
  nc <- if (panel) 1 else length(col_names(x))
  ...
  reps <- diagnostics(x, y, option, panel) %>%
    match(cand)
  ...
  # remove the names that do not pass first stage
  ds_filter <- ds_stamp[reps]

  index_add <- lapply(ds_filter, function(t) data.frame(
      "Start" = index(x)[t[, 1]],
      "End" = index(x)[t[, 2]],
      "Duration" = t[, 3], row.names = NULL
    ))

  min_reject <- lapply(ds_filter, function(t) length(t) == 0) %>% unlist()
  ...
  if (bool) {
    dummy <- matrix(0, length(index(x)), nc,
                    dimnames = list(NULL, if (panel) "Panel" else col_names(x)))
    for (z in 1:nc) {
      dvec <- ds_stamp[[z]]
      dummy[dvec, z] <- 1
    }
  ...
  if (plot_type == "multiple") {
    ...
    if (class(index(x)) == "Date") {
      if (!is.null(breaks_x) && fm) {
        ...
        h <- h + scale_x_date(limits = c(head(index(x), 1), tail(index(x), 1)))
      }
    }else{
      if (!is.null(breaks_x)) {
