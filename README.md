...
paper’s own release). All three of `MultipleBubbles`, `psymonitor` and
`exuber 0.4.1` spike together at `n = 200` – they were timed in the same
interleaved `microbenchmark()` call in that archived run, so a shared
spike points to a one-off system hiccup during that run rather than a
real effect. Only the `exuber 2.0.0` series was freshly simulated, with
