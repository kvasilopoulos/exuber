...
test_that("monitor runs end to end and returns a well-formed object", {
  set.seed(1)
  n <- 100
  y <- cumsum(rnorm(n))
  out <- monitor(y, r_star = 0.5, minw = 20, nboot = 99, seed = 1)
  ...
  expect_output(print(out), "monitor")
})

test_that("'r_star' must leave room for both a training window and at
  least one monitoring observation", {
  y <- cumsum(rnorm(60))
  expect_error(monitor(y, r_star = 5, minw = 20, nboot = 20))
  expect_error(monitor(y, r_star = 60, minw = 20, nboot = 20))
})
...
test_that("an alarm, when raised, always falls strictly after the
  ...
  run_once <- function(seed) {
    ...
    out <- monitor(y, r_star = n1 / length(y), minw = 20, nboot = 99, seed = 1)
...
test_that("monitor detects a clear bubble starting strictly after the
...
test_that("monitor(boundary = 'kurozumi', s0 = 0.4/0.8) runs end to end,
...
test_that("monitor(boundary = 'kurozumi', s0 = 0) is unchanged (s0 = 0
...
test_that("boundary = 'kurozumi', s0 = 0.4 false-alarm rate under H0 is close
  ...
  run <- function(seed) {
    ...
    c(
      sadf = !is.na(unname(monitor(y, r_star = T_star, boundary = "kurozumi", s0 = 0)$alarm)),
      gsadf = !is.na(unname(monitor(y, r_star = T_star, boundary = "kurozumi", s0 = 0.4)$alarm))
    )
  }
