...
`radf_cusum` implements Homm & Breitung (2012)'s CUSUM real-time
...
An object of class `radf_cusum_obj`: a list with the monitoring-region
statistic path (`S`) and `boundary`, the training window length
`T_star`, and `alarm`/`alarm_date` (the first breach, `NA` if none).
