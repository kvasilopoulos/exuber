...
| 1.3 | Advance deprecations (`lifecycle` “communicate” article) | \[x\] | [`radf_wb_cv2()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md)/[`radf_wb_distr2()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md) newly [`.Deprecated()`](https://rdrr.io/r/base/Deprecated.html); [`col_names()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md)/[`mc_cv()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md)/[`wb_cv()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md)/[`sb_cv()`](https://kvasilopoulos.github.io/exuber/reference/exuber-deprecated.md) stay as-is (no removal this cycle) |
...
Local:
`devtools::check(cran = TRUE, remote = TRUE, manual = FALSE, env_vars = c(NOT_CRAN = "false"))`,
R 4.6.1, Windows 11, Rtools45.
...
| 4.8 | “no visible binding for global variable” NOTE | \[x\] | NSE column names registered in `R/zzz.R` [`globalVariables()`](https://rdrr.io/r/utils/globalVariables.html) |
...
| 5.12 | HTTPS for downloads, adequate timeout | \[x\] | `https://exuber.up.railway.app/crit2/<lag>/<n>`; [`download.file()`](https://rdrr.io/r/utils/download.file.html) default timeout |
| 5.13 | No [`.Internal()`](https://rdrr.io/r/base/Internal.html), `:::` on other packages, [`q()`](https://rdrr.io/r/base/quit.html)/`exit()` in compiled code | \[x\] | `rg` audit clean |
...
on a `radf_obj` with no explicit `cv` fetch one table from the store on
first use. Examples, vignettes and tests exercise that path, so on a
...
2.  **Ship the handful of tables the checks need as
    `inst/extdata/crit/`** and have
