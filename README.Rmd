...
data rather than a re-run -- MultipleBubbles/psymonitor are `O(T^2)` pure-R loops 
that already cost minutes per run at `n = 1000`, and none of these archived numbers depend
on the current `exuber` implementation. `exuber 2.0.0` is faster than
`0.4.1` at every sample size shown.
