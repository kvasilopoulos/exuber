...
`0.4.1` at every sample size shown, including `n = 100` (2.1 ms vs.
2.4 ms) -- at that size the call itself takes only ~1-3 ms, so a single
short microbenchmark run is noisy relative to system scheduler jitter;
these numbers are the median of 5 independent blocks of 300 reps each,
stable to within a few percent across repeated attempts.
