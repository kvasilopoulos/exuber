...
input validation, index parsing and attribute/class setup that the
much smaller 0.4.1-era function didn't have. That fixed cost is most of
the runtime at `n = 100` (1.8 of 2.6 ms) but under 4% of it by
`n = 1000`, so the leaner old version wins only while `n` is small
enough that there's barely any real computation to dominate it.
