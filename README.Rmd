...
On default `exuber` will use Monte Carlo simulated critical values if no other option is provided. The package offers these critical values in the form of `data` (up to 600 observations), that are obtain with the `mc_cv()` function.

### Analysis

For the analysis you should include both the `object` and `cv`. The below methods
break the process into small simple steps:
