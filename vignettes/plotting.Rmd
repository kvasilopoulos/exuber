...
The visual output of autoplot in `v0.4.0` is exactly the same the autoplot `v0.3.0`.
...
The custom color for autoplot are "blue and "red", however the user may easily override 
...
`shade_opt` allows the user to manipulate the `geom_rect()` layer of the plot, using
the `shade` function. Additionally, it can be omitted if it set to `NULL`.
